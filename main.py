...
app = Flask(__name__)
MAX_TRADES = 10
bot_state = {"realized": 0.0, "base_capital": 5000, "trades": [], "is_running": True, "btc_trend": "DOWN - دخول SHORT", "btc_change": -0.77}
...
<title>RAIS V25.1 LABEL ABOVE</title>
...
.label-top{font-size:12px!important;color:#aaa;text-align:right;line-height:1.2}
...
.money{font-size:22px!important}
.input-s{background:#000;color:#fff;border:2px solid #444;border-radius:7px;padding:4px;text-align:center;font-size:16px!important;width:70px;height:32px}
.btn-s{border:none;border-radius:7px;padding:5px 12px;font-size:12px!important;cursor:pointer;height:32px}
...
<div style="font-size:17px">👑 RAIS V25.1 - الكتابة فوق الحقل + المستطيل تحته</div>
<button id="togBtn" onclick="toggleBot()" style="background:#00ff88;color:#000;padding:6px 18px;border-radius:40px;font-size:13px!important;border:none">● RUNNING - BTC TRACKING</button>
</div>

<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-top:8px">

<!-- كرت 1 - الرصيد -->
<div class="card" style="background:#0f1410;border-color:#00ff88">
<div class="label-top">💰 الرصيد الاساسي - يتوزع تلقائي ÷10</div>
<div class="value-box" style="border:2px solid #00ff88">
<div style="display:flex;align-items:center;gap:6px"><span style="font-size:18px">💲</span><span class="money" style="color:#00ff88">$<span id="baseShow">5000</span></span><span style="font-size:11px;color:#aaa">→ $<span id="perShow">500</span>/صفقة</span></div>
<div style="display:flex;gap:4px"><input id="baseIn" value="5000" class="input-s" style="border-color:#00ff88;color:#00ff88"><button onclick="saveBase()" class="btn-s" style="background:#00ff88;color:#000">SAVE</button></div>
</div>
</div>

<!-- كرت 2 - الربح العائم -->
<div class="card" style="background:#1a1212;border-color:#ff3b3b">
<div class="label-top">📈 الربح العائم (كل صفقة مستقل)</div>
<div class="value-box" style="border:2px solid #ff3b3b">
<div class="money" style="color:#00ff88" id="flt">$26.25</div>
<div style="text-align:center"><div style="font-size:11px;color:#ffbe0b" id="btcTrend">DOWN - دخول SHORT</div><div style="font-size:11px;color:#ff3b3b" id="btcCh">-0.77%</div></div>
</div>
</div>

<!-- كرت 3 - الصفقات -->
<div class="card" style="background:#121420;border-color:#00d4ff">
<div class="label-top">🏦 صفقات مفتوحة - كل صفقة عايمة حتى ينعكس الاتجاه</div>
<div class="value-box" style="border:2px solid #00d4ff">
<div style="display:flex;align-items:center;gap:6px"><span style="font-size:18px">📊</span><span class="money" style="color:#00d4ff"><span id="openCount">10</span>/10</span></div>
<div style="font-size:10px;color:#aaa;text-align:center">كل صفقة عايمة حتى<br>ينعكس الاتجاه</div>
</div>
</div>
...
<span style="font-size:11px;color:#ffbe0b;margin-left:10px">● BTC توزيع تلقائي مربوط بحركة</span>
</div>

<div style="background:#0e0e14;border:1.5px solid #333;border-radius:10px;padding:8px;margin-top:8px">
<div style="font-size:13px;margin-bottom:4px">LIVE TRADES - صفقات كحد أقصى - كل صفقة مستقلة وعايمة - 10</div>
...
function load(){
 fetch('/api/data?t='+Date.now()).then(r=>r.json()).then(d=>{
  ...
  let h=''; d.trades.forEach(t=>{
    ...
    h+=`<tr><td class="td" style="color:#fff">${t.coin.replace('USDT','')}</td><td class="td" style="color:#ffbe0b">${entryFmt}</td><td class="td" style="color:#00ff88">${liveFmt}</td><td class="td" style="color:#ffbe0b">${t.vol.toFixed(1)}%</td><td class="td">${t.side}</td><td class="td">$${t.cap.toFixed(0)}</td><td class="td" style="color:${cl}">$${t.usd.toFixed(2)}</td><td class="td" style="color:${cl}">${t.pct}%</td></tr>`}); document.getElementById('tbody').innerHTML=h;
 });
}
...
def data():
    ...
    return jsonify({"trades":bot_state["trades"],"floating":sum(t["usd"] for t in bot_state["trades"]),"realized":bot_state["realized"],"base_capital":bot_state["base_capital"],"per_trade":per,"is_running":bot_state["is_running"],"btc_trend":bot_state["btc_trend"],"btc_change":bot_state["btc_change"]})
