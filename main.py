"""
V85.2 AUTO START - يبدأ تلقائي بعد التصفير + زر تشغيل يشغل تلقائي + TradingView REAL
"""
...
config={"capital":1000.0,"per_trade":100.0,"target_pct":0.5,"sl_pct":0.30,"hospital_cap":15}
...
def get_movers(limit=80):
    ...
    coins=["CREAM","PNT","KDA","CLV","MDX","ARK","BTC","ETH","SOL"]; random.shuffle(coins); return [(c,random.uniform(5,70),random.uniform(0.01,100)) for c in coins[:limit]]
...
def init_pos():
    ...
    state["last"]=f"بدأ - {len(state['positions'])} عملات مولعة"
...
def engine():
    ...
    while True:
        try:
            ...
            # يبدأ تلقائي إذا فاضي
            if len(state["positions"])==0 and len(state["treatment"])==0 and time.time()-last_auto>3:
                init_pos()
                last_auto=time.time()
...
def control(cmd):
    if cmd=="toggle":
        ...
        if state["is_running"] and len(state["positions"])==0 and len(state["treatment"])==0:
            init_pos()
    elif cmd=="lock":
        ...
        state["positions"]=[]; state["treatment"]=[]; state["doctor"]={"healed":0,"profit":0.0,"start":time.time(),"rate":95.4,"elapsed":0}
        state["specialty"]=False; state["is_running"]=True; state["last"]="تم التصفير - سيبدأ تلقائي خلال 3 ثواني"
        # يبدأ تلقائي بعد 3 ثواني
        def delayed_start():
            time.sleep(3)
...
@app.route('/api/data')
def api_data():
    total=state["fixed"]+state["safi"]+state["ghair"]-state["loss_pool"]; elapsed=int(time.time()-state["doctor"]["start"]); total_cases=state["doctor"]["healed"]+len(state["treatment"]); rate=round(state["doctor"]["healed"]/total_cases*100 if total_cases else state["doctor"]["rate"],1)
...
.docbar{display:flex;justify-content:space-between;padding:8px 10px;background:#002a00;border:2px solid #00ff66;border-radius:10px;margin:6px;font-size:11px;font-weight:900;color:#00ff66;flex-wrap:wrap;gap:4px}.docbar.spec{background:#2a0000;border-color:#ff3b30;color:#ff9800}
...
<div class="panel"><h3>⚙️ لوحة تحكم رأس المال - نسبة مئوية من رأس المال</h3><div class="ctrls"><div class="ctrl"><label>💰 رأس المال $</label><input id="cap" type="number" value="1000" onchange="save()"></div><div class="ctrl"><label>📦 حجم الصفقة $</label><input id="per" type="number" value="100" onchange="save()"></div><div class="ctrl"><label>🎯 نسبة ربح %</label><input id="targ" type="number" value="0.5" step="0.1" onchange="save()"><div class="val" id="targVal">0.5% من رأس المال</div></div><div class="ctrl"><label>🏥 سعة المستشفى</label><input id="hcap" type="number" value="15" onchange="save()"><div class="val" id="hcapVal">15 مريض = تخصصي</div></div></div></div>
<div class="header"><button class="hbtn orange" onclick="ctrl('try')">🧪 جرب الصيدلية</button><button class="hbtn yellow" onclick="ctrl('reset')">🔄 تصفير - يبدأ تلقائي بعد 3ث</button><button class="hbtn red" onclick="ctrl('lock')">🔒 قفل الكل</button><button class="hbtn green" id="btnRun" onclick="ctrl('toggle')">⏸️ ايقاف</button><button class="hbtn blue" id="btnSpec" onclick="ctrl('specialty')">🏥 تخصصي علاج</button></div>
<div class="cards"><div class="c"><div class="l">💰 ثابت</div><div class="v w" id="f1">1000$</div></div><div class="c ph"><div class="l">📦 الصيدلية</div><div class="v r" id="f2">-0.00$</div><div class="l" id="f2c">0 دواء</div></div><div class="c"><div class="l">💹 صافي</div><div class="v g" id="f3">+0$</div></div><div class="c gold"><div class="l">💎 الإجمالي</div><div class="v g" id="f5">1000$</div></div><div class="c"><div class="l">📈 غير محققة</div><div class="v g" id="f6">+0.00$</div></div></div>
...
async function load(){ try{ const r=await fetch('/api/data'); const d=await r.json(); document.getElementById('cap').value=d.config.capital; document.getElementById('per').value=d.config.per_trade; document.getElementById('targ').value=d.config.target_pct; document.getElementById('hcap').value=d.config.hospital_cap; document.getElementById('elapsed').innerText='⏱️ '+d.elapsed+'ث'; document.getElementById('rate').innerText='📊 نسبة شفاء: '+d.heal_rate+'%'; document.getElementById('profit').innerText='💰 ارباح علاج: +'+d.doctor.profit.toFixed(2)+'$'; document.getElementById('healed').innerText='شفى: '+d.doctor.healed; const bar=document.getElementById('docbar'); if(d.specialty){ bar.className='docbar spec'; document.getElementById('spec').innerText='🔴 تخصصي علاج - المستشفى ممتلئ!'; document.getElementById('btnSpec').innerText='🟢 طبيعي'; } else { bar.className='docbar'; document.getElementById('spec').innerText='🏥 طبيعي - '+d.last_healed; document.getElementById('btnSpec').innerText='🏥 تخصصي علاج'; } document.getElementById('f1').innerText=d.fixed.toFixed(0)+'$'; document.getElementById('f2').innerText=d.loss_pool.toFixed(2)+'$'; document.getElementById('f2c').innerText=d.treatment.length+' دواء'; document.getElementById('f3').innerText='+'+d.safi.toFixed(0)+'$'; document.getElementById('f5').innerText=d.total+'$'; document.getElementById('f6').innerText='+'+d.ghair.toFixed(2)+'$'; document.getElementById('btnRun').innerText=d.is_running?'⏸️ ايقاف':'▶️ تشغيل'; document.getElementById('src').innerText='📡 '+d.data_source+(d.specialty?' | 🔴 تخصصي':' | 🟢 طبيعي'); document.getElementById('bin').innerText=d.binance_status; document.getElementById('time').innerText=d.last_update; let h=''; for(const p of d.positions){ h+=`<div class="row" style="grid-template-columns:1.2fr 0.8fr 1.5fr 1fr 1fr 1fr 0.8fr"><div style="color:#fff;font-weight:900">${p[0]}</div><div><span class="badge green">${p[1]}</span></div><div style="color:#00ff66">${p[6]}</div><div>${p[2].toFixed(4)}</div><div>${p[3].toFixed(4)}</div><div style="color:#00ff66">${p[4].toFixed(3)}$</div><div style="color:#00ff66">${p[5].toFixed(1)}%</div></div>` } document.getElementById('plist').innerHTML=h||'<div style="padding:10px;text-align:center">⏳ جاري جلب عملات مولعة...</div>'; let t=''; for(const p of d.treatment){ let prog=Math.min(100,Math.max(0,(p[4]/p[9]*100)||5)); t+=`<div class="row" style="grid-template-columns:1fr 1.5fr 0.8fr 0.8fr 0.8fr 1fr;background:#1a0f00"><div><span class="badge orange">${p[0]}</span></div><div style="color:#ffeb3b">${p[7]}</div><div style="color:#ff3b30">-${p[8].toFixed(2)}$</div><div style="color:#ffeb3b">${p[9].toFixed(2)}$</div><div>${p[3].toFixed(4)}</div><div style="color:#00ff66">${prog.toFixed(0)}%</div></div>` } document.getElementById('tlist').innerHTML=t||'<div style="padding:10px;text-align:center;background:#1a0f00">المستشفى فاضي ✅</div>'; }catch(e){} } setInterval(load,1000); load();
</script></body></html>"""
