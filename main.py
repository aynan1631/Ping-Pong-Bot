"""
V88.1 FIX - هدف دولار ثابت يقبل التغيير + يحط في صافي محقق
"""
...
def get_hot_coins():
    try:
        ...
        state["data_source"]=f"هدف {config['target_dollar']}$ قفل"
        return hot[:15]
    except: return []
    ...
    while True:
        try:
            ...
            if state["ghair"] >= config["target_dollar"] and len(state["positions"])>0:
                ...
                state["last"]=f"✅ قفل {profit:.3f}$ -> صافي"
            ...
            if len(state["positions"]) < config["max_pos"] and not state["specialty"]:
                ...
                for sym,pct,price,vol,power,bull in hot:
                    if sym not in exist and len(state["positions"])<config["max_pos"]:
                        tag=f"مولعة {pct:.1f}%"; state["positions"].append([sym,"SPOT",price*0.9995,price,0.0,0.0,tag,time.time()]); exist.add(sym)
...
def set_config():
    ...
    try:
        ...
        # يقبل target أو target_dollar أو حتى لو جاي % يحوله لدولار
...
def api_data():
    ...
    return jsonify({"fixed":state["fixed"],"safi":state["safi"],"ghair":state["ghair"],"total":round(total,3),"trades_closed":state["trades_closed"],"loss_pool":state["loss_pool"],"positions":state["positions"],"treatment":state["treatment"],"binance_status":state["binance_status"],"data_source":state["data_source"],"is_running":state["is_running"],"doctor":state["doctor"],"elapsed":elapsed,"heal_rate":95.4,"specialty":state["specialty"],"last_healed":state["last"],"config":config})
...
<div class="docbar" id="docbar"><span id="elapsed">⏱️ 0ث</span><span id="rate">📊 شفاء: 95.4%</span><span id="profit">💰 علاج: +0.00$</span><span id="healed">شفى: 0</span><span id="spec">🏥 طبيعي 0.5$ قفل</span></div>
<div class="panel"><h3>⚙️ V88.1 FIX - هدف نص دولار -> حانة الربح 💰</h3><div class="ctrls"><div class="ctrl"><label>💰 رأس المال $</label><input id="cap" type="number" value="1000" onchange="save()"></div><div class="ctrl"><label>📦 حجم الصفقة $</label><input id="per" type="number" value="100" onchange="save()"></div><div class="ctrl"><label>🎯 هدف قفل $ (يقبل التغيير)</label><input id="targ" type="number" value="0.5" step="0.1" min="0.1" onchange="save()"><div style="font-size:10px;color:#00ff66;font-weight:900" id="targVal">0.5$ -> يقفل الكل للصافي</div></div><div class="ctrl"><label>🏥 سعة المستشفى</label><input id="hcap" type="number" value="15" onchange="save()"></div></div></div>
<div class="header"><button class="hbtn orange" onclick="ctrl('try')">🧪 دخول MACD</button><button class="hbtn yellow" onclick="ctrl('reset')">🔄 تصفير</button><button class="hbtn red" onclick="ctrl('lock')">🔒 قفل الآن -> صافي</button><button class="hbtn green" id="btnRun" onclick="ctrl('toggle')">⏸️ ايقاف</button><button class="hbtn blue" id="btnSpec" onclick="ctrl('specialty')">🏥 تخصصي</button></div>
<div class="cards"><div class="c"><div class="l">💰 ثابت</div><div class="v w" id="f1">1000$</div></div><div class="c ph"><div class="l">📦 الصيدلية</div><div class="v r" id="f2">-0.00$</div><div class="l" id="f2c">0 دواء</div></div><div class="c" style="border:2px solid #00ff66"><div class="l">💹 صافي محقق - حانة الربح</div><div class="v g" id="f3" style="font-size:18px">+0$</div></div><div class="c gold"><div class="l">💎 الإجمالي</div><div class="v g" id="f5">1000$</div></div><div class="c"><div class="l">📈 غير محققة</div><div class="v g" id="f6">+0.00$</div></div></div>
