"""
V85 FINAL CLEAN - بدون شريط المتصفح + لوحة تحكم + مستشفى + TradingView REAL
"""
...
state={"fixed":1000.0,"safi":137.0,"ghair":0.66,"trades_closed":874,"loss_pool":0.71,"positions":[],"treatment":[],"binance_status":"✅ حقيقي","data_source":"TradingView","is_running":True,"doctor":{"healed":54,"profit":35.80,"start":time.time(),"rate":95.4,"elapsed":124},"specialty":False,"last":"CREAM +0.50$"}
...
def init_pos():
    ...
    for sym,pct,price in mov[:3]: state["positions"].append([sym,"SPOT",price*0.9995,price,0.050,65.4,f"مولعة {pct:.1f}%",time.time()])
    if len(mov)>=2: state["treatment"]=[[mov[3][0],"علاج",mov[3][2]*0.9995,mov[3][2],0.050,0.35,0.35,f"{mov[3][0]} يعالج 0.35$",0.35,0.85],[mov[4][0] if len(mov)>4 else "MDX","علاج",0.0350,0.0350,0.050,0.36,0.36,"يعالج 0.36$",0.36,0.86]]
...
def engine():
    while True:
        try:
            ...
            state["ghair"]=round(sum(p[4] for p in state["positions"]),3); state["loss_pool"]=round(sum(p[8] for p in state["treatment"]),3)
...
def control(cmd):
    ...
    elif cmd=="reset": state["safi"]=0; state["ghair"]=0; state["trades_closed"]=0; state["treatment"]=[]; state["doctor"]={"healed":0,"profit":0.0,"start":time.time(),"rate":95.4,"elapsed":0}; init_pos()
    elif cmd=="try":
        mov=get_movers(20)
...
def set_config():
    d=request.json; config["capital"]=float(d.get("capital",config["capital"])); config["per_trade"]=float(d.get("per_trade",config["per_trade"])); config["target_pct"]=float(d.get("target",config["target_pct"])); config["hospital_cap"]=int(d.get("hcap",config["hospital_cap"])); return jsonify({"ok":True,"config":config})
...
<!-- شريط الطبيب بدون شريط المتصفح -->
<div class="docbar" id="docbar"><span id="elapsed">⏱️ 124ث</span><span id="rate">📊 نسبة شفاء: 95.4%</span><span id="profit">💰 ارباح علاج: +35.80$</span><span id="healed">شفى: 54</span><span id="spec">🏥 طبيعي</span></div>

<!-- لوحة تحكم رأس المال -->
<div class="panel">
<h3>⚙️ لوحة تحكم رأس المال والربح</h3>
<div class="ctrls">
