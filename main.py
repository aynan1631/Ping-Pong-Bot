...
app = Flask(__name__)
MAX_TRADES = 10
bot_state = {"realized": 0.0, "base_capital": 5000, "trades": [], "is_running": True, "btc_trend": "DOWN - دخول SHORT", "btc_change": -0.71}
...
HTML="""<!DOCTYPE html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>RAIS V25.4</title>
...
<div style="display:flex;align-items:center;gap:4px"><span style="font-size:15px;color:#fff">$</span><span class="money" style="color:#00ff88">$<span id="baseShow">5000</span></span><div style="display:flex;flex-direction:column;line-height:1"><span style="font-size:10px;color:#aaa">500$/</span><span style="font-size:10px;color:#aaa">صفقة</span></div></div>
<div style="display:flex;gap:5px;align-items:center"><input id="baseIn" value="5000" class="input-s" style="border-color:#00ff88;color:#00ff88"><button onclick="saveBase()" class="btn-s" style="background:#00ff88;color:#000">SAVE</button></div>
</div>
</div>

<!-- العائم -->
<div class="card" style="background:#1a1212;border-color:#ff3b3b">
<div class="label-top">📈 الربح العائم (كل صفقة مستقل) - يتصفر مع التحديث</div>
<div class="value-box" style="border:2px solid #ff3b3b">
<div class="money" id="flt" style="color:#ff3b3b">-$11.10</div>
<div style="text-align:center;line-height:1.2"><div style="font-size:11px;color:#ffbe0b" id="btcTrend">DOWN - دخول SHORT</div><div style="font-size:11px;color:#ff3b3b;margin-top:2px" id="btcCh">-0.71%</div></div>
</div>
</div>

<!-- المحقق -->
<div class="card" style="background:#121420;border-color:#00d4ff">
<div class="label-top">🏦 الربح المحقق - يتصفر مع اعادة التشغيل</div>
<div class="value-box" style="border:2px solid #00d4ff">
<div style="display:flex;align-items:center;gap:6px"><span style="font-size:18px">💎</span><span class="money" style="color:#00d4ff">$<span id="real">0.00</span></span></div>
<div style="display:flex;flex-direction:column;gap:4px"><button onclick="resetReal()" style="background:#222;color:#fff;border:1px solid #444;border-radius:6px;padding:3px 8px;font-size:9px;cursor:pointer">RESET 0</button><button onclick="closeAll()" style="background:#00d4ff;color:#000;border:none;border-radius:6px;padding:4px 8px;font-size:9px;cursor:pointer">قفل الصفقات → محقق</button></div>
</div>
</div>
