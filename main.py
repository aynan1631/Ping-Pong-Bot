"""
V100 LEGEND DUAL - رأس مال أوتوماتيك + عدادات شغالة
"""
        ...
        try:
            ...
            if state["max_safi"] > 50:
                if state["safi"] < state["max_safi"] * (1 - config["protect_pct"]) and not state["protect_triggered"]:
                    state["is_running"] = False; state["protect_triggered"] = True
    ...
    if cmd=="toggle":
        ...
        try:
            if REAL_CLIENT:
                ...
                bal=float(b['free'])
                if bal>=0:
                    config["capital"]=bal if bal>0.5 else config["capital"]
                    state["real_balance"]=f"{bal:.2f}"
                    if bal>0.5:
                        state["fixed"]=bal+state["safi"]
        except: pass
        state["binance_status"]=f"V100 REAL {config['capital']:.2f}$"
    return jsonify({"ok":True, "capital": config["capital"], "mode": state["mode"]})
...
<div class="panel"><h3 id="mainTitle">👑 V100 DUAL - عدادات مضبوطة 👑</h3><div class="grid">
...
if(document.activeElement.id!=='cap'){
  document.getElementById('cap').value=en(d.config.capital,0);
}
if(d.mode=='REAL'){
  document.getElementById('capLabel').innerText='💰 راس المال $ 👑 حقيقي';
  document.getElementById('capInfo').innerText='👑 '+d.real_balance+'$ من BINANCE';
} else {
  document.getElementById('capLabel').innerText='💰 راس المال $ 🧪 تجريبي';
  document.getElementById('capInfo').innerText='🧪 '+d.test_balance+'$ من TESTNET';
}
