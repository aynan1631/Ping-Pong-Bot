"""
V100 LEGEND DUAL - النسخة النهائية المضبوطة
تجريبي 10000$ أوتوماتيك + حقيقي أوتوماتيك
"""
...
state={"fixed":10000.0,"safi":0.0,"max_safi":0.0,"ghair":0.0,"trades_closed":0,"loss_pool":0.0,"positions":[],"treatment":[],"doctor_positions":[],"binance_status":f"V100 DUAL - {DEFAULT_MODE}","data_source":f"V100 DUAL {DEFAULT_MODE}","is_running":True,"mode":DEFAULT_MODE,"real_balance":"--","test_balance":"10000.00","doctor":{"healed":0,"profit":0.0,"start":time.time(),"rate":99.5,"active_patient":None,"active_doctor":None},"specialty":False,"last":"V100 DUAL","healing_mode":False,"protect_triggered":False}
...
def update_balances():
    try:
        if REAL_CLIENT:
            ...
            if state["mode"]=="REAL" and bal>0.5:
                config["capital"]=bal
    ...
    try:
        if TEST_CLIENT:
            ...
            if state["mode"]=="TESTNET" and bal>1:
                config["capital"]=bal
                state["fixed"]=bal+state["safi"]
    except:
    ...
    while True:
        try:
            ...
            if len(state["treatment"]) >= config["hospital_cap"]: state["specialty"]=True; state["binance_status"]=f"زحمة {len(state['treatment'])}/30 [{state['mode']}]"
                ...
                else: state["binance_status"]=f"V100 {state['mode']} {len(state['positions'])}/8 حجم {dyn:.0f}$ اعلى {state['max_safi']:.0f}$"
...
<div class="mode-bar">
<button id="btnTest" class="mode-btn" onclick="ctrl('mode_test')" style="background:#0A1F3A;color:#22D3EE;border-color:#22D3EE">🧪 تجريبي TESTNET<br><span id="testBal" style="font-size:11px">--</span></button>
<button id="btnReal" class="mode-btn" onclick="ctrl('mode_real')" style="background:#1A1000;color:#D4AF37;border-color:#D4AF37">👑 حقيقي LEGEND<br><span id="realBal" style="font-size:11px">--</span></button>
</div>
<div class="panel"><h3 id="mainTitle">👑 V100 DUAL - رأس مال أوتوماتيك 👑</h3><div class="grid">
<div class="box" style="border:2px solid #D4AF37"><label>💰 راس المال $ 🔒 أوتوماتيك</label><div class="step-row"><input id="cap" type="text" value="10000$" readonly style="background:#1A1000;color:#D4AF37;border:2px solid #D4AF37;font-size:16px;font-weight:900"><button class="step-btn" onclick="refreshCap()" style="font-size:10px;width:55px">تحديث</button></div><div id="capInfo" style="font-size:9px;color:#D4AF37;margin-top:3px">من بينانس مباشرة</div></div>
