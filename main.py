"""
V100 LEGEND DUAL - تجريبي + حقيقي بزرين
نفس منطق V100 + نظام مزدوج
"""
from flask import Flask, jsonify, request
import threading, time, os, requests
app = Flask(__name__)

# ===== إعداد المفاتيح المزدوجة =====
try:
...
def update_balances():
    try:
        if REAL_CLIENT:
            ...
            state["real_balance"]=f"{float(b['free']):.2f}"
    except: state["real_balance"]="خطأ"
    try:
        if TEST_CLIENT:
            b=TEST_CLIENT.get_asset_balance(asset='USDT')
            state["test_balance"]=f"{float(b['free']):.2f}"
    except: state["test_balance"]="خطأ"
...
def control(cmd):
    ...
    elif cmd=="mode_test": state["mode"]="TESTNET"; state["binance_status"]=f"V100 TESTNET 🧪"
    elif cmd=="mode_real": state["mode"]="REAL"; state["binance_status"]=f"V100 REAL 👑"
