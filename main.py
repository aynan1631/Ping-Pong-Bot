...
def get_trend_tf(sym, interval):
    try:
        ...
        if closes[-1] > sma20 and closes[-1] > sma50: return "UP"
        if closes[-1] < sma20 and closes[-1] < sma50: return "DOWN"
        return None
    except:
        return None
...
def get_btc_all_tf():
    try:
        return {tf: get_trend_tf("BTCUSDT", tf) for tf in ["5m","15m","1h","4h"]}
    except:
        return {"4h":"UP","1h":"UP"}
