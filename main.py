...
app = Flask(__name__)
MAX_TRADES = 10
# مع كل تحديث يتصفر - يبدأ من صفر
bot_state = {"realized": 0.0, "base_capital": 5000, "trades": [], "is_running": True, "btc_trend": "DOWN - دخول SHORT", "btc_change": -0.89}
...
HTML="""<!DOCTYPE html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>RAIS V25.3</title>
<link href="https://fonts.googleapis.com/css2?family=Cairo:wght@700;800&display=swap" rel="stylesheet">
<style>*{font-family:'Cairo',sans-serif!important;font-weight:800!important;box-sizing:border-box}body{background:#07070a;margin:0;padding:8px;color:#fff}
.card{border-radius:12px;padding:10px 12px;border:2.5px solid;display:flex;flex-direction:column;gap:6px;height:96px}
.label-top{font-size:12px!important;color:#aaa;text-align:center}
.value-box{border-radius:8px;padding:6px 10px;display:flex;align-items:center;justify-content:space-between;background:rgba(0,0,0,0.4);height:46px}
.money{font-size:22px!important}.input-s{background:#000;color:#fff;border:2px solid #444;border-radius:7px;padding:4px;text-align:center;font-size:16px!important;width:70px;height:32px}
.btn-s{border:none;border-radius:7px;padding:5px 12px;font-size:12px!important;cursor:pointer;height:32px}.th{font-size:11px!important;color:#ffbe0b!important;padding:6px 3px!important}.td{font-size:12px!important;padding:5px 3px!important;border-top:1px solid #1a1a22!important;text-align:center}</style></head><body>
...
def close_all():
    ...
    closed = len(bot_state["trades"])
    bot_state["realized"] += floating
    bot_state["trades"] = []
    return jsonify({"ok":True,"closed":closed,"floating":floating,"realized":bot_state["realized"]})
