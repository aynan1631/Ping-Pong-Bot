"""
V98.5 BAN - نسختك نفسها + حظر ASTR/ASTAR/SAGA/FF + منع الطبيب يعالج نفسه
التعديل على نسختك V98.1 اللي ارسلتها
"""
...
# 🚫 قائمة الحظر النهائي - ما يدخلون ابدا
        ...
        try:
            ...
            for p in state["treatment"][:]:
                if p[0] in BANNED:
                    ...
                    state["last"]=f"🚫 طرد محظور {p[0]} من المستشفى"
            for p in state["positions"][:]:
                if p[0] in BANNED:
                    state["positions"].remove(p)
                    state["last"]=f"🚫 طرد محظور {p[0]} من الصيدلية"
            for d in state["doctor_positions"][:]:
                if d[0] in BANNED or d[9] in BANNED:
                    state["doctor_positions"].remove(d)
                    state["last"]=f"🚫 طرد طبيب محظور {d[0]}->{d[9]}"
            for p in state["positions"]:
                try:
            ...
            for d in state["doctor_positions"][:]:
                if d[0] == d[9]:
                    state["doctor_positions"].remove(d)
                    state["last"]=f"طرد {d[0]} يعالج نفسه - V98.5 BAN"
            ...
            if config["doctor_enabled"] and len(state["treatment"]) > 0 and len(state["doctor_positions"]) < config["max_doctors"]:
                ...
                for strongest in strongest_list:
                    ...
                    for t in state["treatment"]:
                        if t[0] not in used_patients and t[0] not in BANNED:
                    ...
                    if sym in BANNED or oldest[0] in BANNED: continue
            ...
            if len(state["treatment"]) >= config["hospital_cap"]:
                ...
                state["binance_status"]=f"شغال {len(state['positions'])}/8 - مستشفى {len(state['treatment'])}/30 - V98.5 BAN"
...
def control(cmd):
    if cmd=="toggle": state["is_running"]=not state["is_running"]
        ...
        state["safi"]=0.0; state["fixed"]=config["capital"]; state["last"]="تم تصفير الصافي يدويا - V98.5"
    elif cmd=="try":
        hot=get_binance_hot()
        ...
        state["healing_mode"]=False; state["doctor_positions"]=[]; state["last"]="FORCE HEAL بدون خصم - V98.5"
    elif cmd=="ban_clean":
        for p in state["treatment"][:]:
        ...
        state["last"]=f"🚫 تم حظر وطرد {BANNED} - V98.5 بدون خصم"
    elif cmd=="force_heal_astr":
        for p in state["treatment"][:]:
            if p[0] in BANNED: state["treatment"].remove(p)
        for d in state["doctor_positions"][:]:
            if d[0] in BANNED or d[9] in BANNED: state["doctor_positions"].remove(d)
        state["last"]="تم فك المحظورين - V98.5"
    elif cmd=="doctor":
...
def api_data():
    ...
    return jsonify({"fixed":state["fixed"],"safi":state["safi"],"ghair":state["ghair"],"total":round(total,3),"trades_closed":state["trades_closed"],"loss_pool":state["loss_pool"],"positions":state["positions"],"treatment":state["treatment"],"doctor_positions":state["doctor_positions"],"binance_status":state["binance_status"],"data_source":f"V98.5 BAN {len(state['treatment'])}/{config['hospital_cap']}","is_running":state["is_running"],"doctor":state["doctor"],"elapsed":elapsed,"heal_rate":99.5,"specialty":state["specialty"],"last_healed":state["last"],"config":config,"healing_mode":state["healing_mode"]})
...
<div class="top" id="top"><span id="elapsed" lang="en" dir="ltr">0s</span><span id="rate" lang="en" dir="ltr">V98.5 - 0/30</span><span id="profit" lang="en" dir="ltr">+0.00$</span><span id="healed" lang="en" dir="ltr">0</span><span id="spec">جاهز</span></div>
<div class="panel"><h3>👑 V98.5 BAN - حظر SAGA/FF/ASTR نهائي 🚫</h3>
<div class="grid">
...
<div class="btns">
<button class="btn" style="background:#000;color:#FF3344;border:2px solid #FF3344" onclick="ctrl('ban_clean')">🚫 طرد المحظورين SAGA/FF/ASTR</button>
...
<div class="foot"><span lang="en" dir="ltr" id="src">V98.5 BAN</span><span lang="en" dir="ltr" id="bin">...</span><span lang="en" dir="ltr" id="time">...</span></div>
<script>
...
async function load(){ try{ const r=await fetch('/api/data'); const d=await r.json();
  ...
  document.getElementById('elapsed').innerText=en(d.elapsed,0)+'s'; document.getElementById('rate').innerText='V98.5 - '+en(d.treatment.length,0)+'/'+en(d.config.hospital_cap,0); document.getElementById('profit').innerText='+'+en(d.doctor.profit,2)+'$'; document.getElementById('healed').innerText=en(d.doctor.healed,0)+' شفى';
