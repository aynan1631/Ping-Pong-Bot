...
app = Flask(__name__)
MAX_TRADES = 10
bot_state = {"realized": 125.40, "base_capital": 5000, "trades": [], "is_running": True, "btc_trend": "DOWN - دخول SHORT", "btc_change": -0.62}
...
HTML="""<!DOCTYPE html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>RAIS V25.2 FINAL FIX</title>
...
<div style="font-size:16px">👑 RAIS V25.2 - الرصيد + العائم + المحقق</div><button id="togBtn" onclick="toggleBot()" style="background:#00ff88;color:#000;padding:6px 18px;border-radius:40px;font-size:13px!important;border:none">● RUNNING - BTC TRACKING</button></div>
...
<!-- 2 - الربح العائم -->
<div class="card" style="background:#1a1212;border-color:#ff3b3b"><div class="label-top">📈 الربح العائم (كل صفقة مستقل)</div><div class="value-box" style="border:2px solid #ff3b3b"><div class="money" id="flt" style="color:#00ff88">$7.25</div><div style="text-align:center"><div style="font-size:11px;color:#ffbe0b" id="btcTrend">DOWN - دخول SHORT</div><div style="font-size:11px;color:#ff3b3b" id="btcCh">-0.62%</div></div></div></div>

<!-- 3 - الربح المحقق (رجع مكان عدد الصفقات) -->
<div class="card" style="background:#121420;border-color:#00d4ff"><div class="label-top">🏦 الربح المحقق - يتصفر مع اعادة التشغيل</div><div class="value-box" style="border:2px solid #00d4ff"><div style="display:flex;align-items:center;gap:6px"><span style="font-size:18px">💎</span><span class="money" style="color:#00d4ff">$<span id="real">0.00</span></span></div><div style="text-align:center"><button onclick="resetReal()" style="background:#222;color:#fff;border:1px solid #444;border-radius:6px;padding:4px 10px;font-size:10px;cursor:pointer">RESET 0</button><div style="font-size:9px;color:#888;margin-top:2px">محقق فعلي</div></div></div></div>

</div>

<div style="display:flex;gap:5px;margin-top:8px;align-items:center"><button onclick="setBase(1000)" class="btn-s" style="background:#222;color:#fff">1000$ → 100$</button><button onclick="setBase(2000)" class="btn-s" style="background:#222;color:#fff">2000$ → 200$</button><button onclick="setBase(5000)" class="btn-s" style="background:#00ff88;color:#000">5000$ → 500$</button><button onclick="setBase(10000)" class="btn-s" style="background:#222;color:#fff">10000$ → 1000$</button><span style="font-size:11px;color:#ffbe0b;margin-left:10px">● BTC توزيع تلقائي مربوط بحركة</span></div>

<div style="background:#0e0e14;border:1.5px solid #333;border-radius:10px;padding:8px;margin-top:8px">
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px"><div style="font-size:13px">LIVE TRADES - صفقات كحد أقصى - كل صفقة مستقلة وعايمة - 10</div><div style="background:#111a30;border:2px solid #00d4ff;border-radius:8px;padding:4px 12px;display:flex;gap:8px;align-items:center"><span style="font-size:12px;color:#aaa">صفقات مفتوحة</span><span style="font-size:16px;color:#00d4ff" id="openCount">10/10</span><span style="font-size:10px;color:#aaa">كل صفقة عايمة حتى ينعكس الاتجاه</span></div></div>
