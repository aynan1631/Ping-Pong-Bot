...
def get_movers():
    base=["TIA","PEPE","POWR","BONK","XRP","VTHO","STEEM","FLOKI","BOME","WIF","ARK","DOGE","SHIB","NOT","SEI"]
    mov=[]
    for s in base:
        mov.append((f"{s}/USDT", random.uniform(10,38), random.uniform(0.0002,3)))
    ...
    while True:
        try:
            ...
            for p in state["positions"]:
                cur = p[2]*(1+random.uniform(-0.005,0.015))
                p[2]=cur
                pp=(cur-p[1])/p[1]*100
...
.lb{font-size:9px;opacity:0.5;font-weight:800}.val{font-family:JetBrains Mono;font-size:28px;font-weight:800;margin-top:4px;transition:all 0.3s}.sub{font-size:11px;opacity:0.7;margin-top:6px;font-weight:700}
...
.coin{background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);border-radius:14px;padding:10px;transition:all 0.3s}
...
.cprice{font-family:JetBrains Mono;font-size:10px;opacity:0.4;direction:ltr;margin:6px 0}.barw{height:7px;background:#000;border-radius:20px;overflow:hidden}.bar{height:100%;border-radius:20px;transition:width 0.5s}.cprof{margin-top:6px;border-radius:8px;padding:6px;text-align:center;font-family:JetBrains Mono;font-weight:800;font-size:13px}.cprof small{display:block;font-size:9px;opacity:0.6}
@media(max-width:900px){.cards{grid-template-columns:1fr 1fr}.panel{grid-template-columns:1fr}.val{font-size:22px}}
</style></head><body>
<div class="top"><span>V12.3 LIVE AUTO • <span id="clock"></span></span><span style="color:#00ff9d">● LIVE - تتحرك تلقائيا كل ثانية</span></div>
<div class="cards">
...
async function save(){
  ...
  const b=document.querySelector('.btn'); const s=document.getElementById('status'); b.innerText='⏳...';
  try{const r=await fetch('/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)}); const j=await r.json(); if(j.ok){b.innerText='✅ تم'; s.innerText='تم الحفظ - تتحرك الآن!'; setTimeout(()=>{b.innerText='حفظ فوري ⚡'; s.innerText='';},1500);} else{b.innerText='❌';}}catch(e){b.innerText='❌';}
}
