...
app = Flask(__name__)
cooldown = {}
# V74 TURBO - TP 0.45 / Target 0.75
state={"total_capital":2000.0,"per_trade":100.0,"balance":2000.0,"realized":0.0,"unrealized":0.0,"total_target":0.75,"trades":[],"TP":0.45,"SL":-0.65,"cycles":0}
...
def try_add_fast():
    ...
    try:
        ...
        for d in data:
            ...
            state["trades"].append(t); have.add(sym)
        recalc_balance()
        return True
    except: return False
...
HTML="""<!DOCTYPE html><html dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>V74 TURBO</title><link href="https://fonts.googleapis.com/css2?family=Cairo:wght@700;900&family=JetBrains+Mono:wght@800&display=swap" rel="stylesheet"><style>
*{font-family:'Cairo',sans-serif;box-sizing:border-box}
body{background:radial-gradient(1200px 600px at 50% -10%, #2a1a0a 0%, #1a1105 30%, #07091a 100%);color:#fff;margin:0;padding:14px}
.header{font-size:26px;font-weight:900;text-align:center;background:linear-gradient(90deg,#ff3d00,#ffd700,#ff6d00);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.sub{font-size:12px;color:#ffb86a;text-align:center;font-weight:800;margin:6px 0 14px}
.glass{background:linear-gradient(145deg,rgba(60,35,15,0.9),rgba(30,18,5,0.95));border:1.5px solid #ff980055;border-radius:20px;padding:14px}
.ctrl-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px}
.ctrl-title{color:#ff9800;font-weight:900;margin-bottom:8px;font-size:13px}
input{background:#07091e;color:#ffcc00;border:2.5px solid #ff9800aa;border-radius:12px;padding:11px 10px;font-size:18px;font-weight:900;text-align:center;direction:ltr;min-width:90px}
.btn{padding:11px 16px;border-radius:12px;border:0;font-weight:900;font-size:13px;cursor:pointer}
.btn-gold{background:linear-gradient(145deg,#ff9800,#ff3d00);color:#fff}
...
.btn-dark{background:linear-gradient(145deg,#3a2a1a,#1e1505);color:#fff;border:1.5px solid #ffffff22}
.dashboard{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:14px 0}
.cardx{background:linear-gradient(145deg,#3d2a15,#2a1c0a);border:1.8px solid #ffffff18;border-radius:20px;padding:14px;text-align:center}
.cardx.gold{border-color:#ff9800aa;box-shadow:0 0 30px #ff980022}
.cardx.profit{border-color:#ff6d00;box-shadow:0 0 35px #ff6d0055}
.lbl{color:#ffb86a;font-size:10.5px;font-weight:800}
...
.table-wrap{background:linear-gradient(145deg,#2a1c0a,#1a1105);border-radius:20px;overflow:hidden;border:1.5px solid #ffffff18;margin-top:12px}
table{width:100%;border-collapse:collapse}
th{background:linear-gradient(145deg,#4a3015,#2e1f0a);color:#ff9800;padding:12px 6px;font-size:12px;font-weight:900;border-bottom:2.5px solid #ff980055}
...
.pill{display:inline-flex;align-items:center;gap:6px;background:#2a1c0a;padding:10px 14px;border-radius:14px;border:1.5px solid #ffffff15}
</style></head><body>
<div class="header">🔥 V74 TURBO — وضع التيربو</div>
<div class="sub">TP 0.45$ | هدف 0.75$ | SL -0.65$ | ANTI-LOSS -5$ | النسخة الذهبية +10.36$ محفوظة</div>
<div class="ctrl-grid">
<div class="glass"><div class="ctrl-title">💰 رأس المال</div><div style="display:flex;gap:8px;justify-content:center"><input id="totalInp" value="{{s.total_capital}}"><button class="btn btn-gold" onclick="fetch('/set_total?v='+document.getElementById('totalInp').value).then(()=>location.reload())">تطبيق</button></div></div>
...
<div class="cardx profit"><div class="lbl">💵 صافي ربح</div><div class="val" id="real" style="color:#00ff88"><span class="ltr">{{'%+.2f'|format(s.realized)}}$</span></div></div>
<div class="cardx"><div class="lbl">📈 غير محققة</div><div class="val" id="unreal"><span class="ltr">{{'%+.2f'|format(s.unrealized)}}$</span></div></div>
...
<div class="pill"><span style="color:#ff9800;font-weight:900">🎯 هدف</span><input id="t" style="width:75px" value="{{s.total_target}}"><button class="btn btn-green" onclick="fetch('/set_target?v='+document.getElementById('t').value).then(()=>location.reload())">حفظ</button></div>
