"""
V86 MACD + سيولة + صافي ربح + مستشفى - آلية الدخول الحقيقية
"""
...
config={"capital":1000.0,"per_trade":100.0,"target_pct":0.5,"sl_pct":0.8,"hospital_cap":15,"max_pos":8,"min_vol":5000000}
state={"fixed":1000.0,"safi":0.0,"ghair":0.0,"trades_closed":0,"loss_pool":0.0,"positions":[],"treatment":[],"binance_status":"✅ MACD حقيقي","data_source":"Binance+MACD","is_running":True,"doctor":{"healed":0,"profit":0.0,"start":time.time(),"rate":95.4},"specialty":False,"last":"ينتظر MACD"}
...
def get_macd_signal(symbol):
    try:
        # جلب شموع 15 دقيقة - 100 شمعة
        r=requests.get(f"https://api.binance.com/api/v3/klines?symbol={symbol}USDT&interval=15m&limit=100", timeout=5)
        if r.status_code!=200: return False, 0
        klines=r.json(); closes=[float(k[4]) for k in klines]; volumes=[float(k[5]) for k in klines]
        if len(closes)<35: return False, 0
        # MACD 12,26,9
        ema12=ema(closes,12); ema26=ema(closes,26)
        if not ema12 or not ema26: return False, 0
        macd_line=ema12-ema26
        # Signal = EMA9 of MACD - نبسطها بـ EMA9 للأسعار الأخيرة
        macd_series=[]
        for i in range(26, len(closes)):
            e12=ema(closes[i-12:i],12) if i>=12 else None
            e26=ema(closes[i-26:i],26)
            if e12 and e26: macd_series.append(e12-e26)
        if len(macd_series)<9: return False, 0
        signal=ema(macd_series,9)
        if not signal: return False, 0
        # تقاطع صاعد + سيولة
        is_bullish = macd_line > signal and macd_series[-1] > macd_series[-2]
        avg_vol = sum(volumes[-10:])/10
        return is_bullish and avg_vol>100, macd_line-signal
    except: return False, 0

def get_movers_macd(limit=50):
    try:
        r=requests.get("https://api.binance.com/api/v3/ticker/24hr", timeout=6)
        if r.status_code==200:
            all_tickers=[t for t in r.json() if t["symbol"].endswith("USDT") and float(t["quoteVolume"])>config["min_vol"] and float(t["priceChangePercent"])>3]
            all_tickers.sort(key=lambda x: (float(x["priceChangePercent"])*0.7 + float(x["quoteVolume"])/10000000*0.3), reverse=True)
            candidates=[]
            for t in all_tickers[:20]: # نفحص أول 20 بس عشان السرعة
                sym=t["symbol"].replace("USDT","")
                pct=float(t["priceChangePercent"]); vol=float(t["quoteVolume"]); price=float(t["lastPrice"])
                is_macd, hist = get_macd_signal(t["symbol"].replace("USDT",""))
                if is_macd:
                    candidates.append((sym,pct,price,vol,hist))
                    if len(candidates)>=limit: break
            candidates.sort(key=lambda x: x[4], reverse=True) # ترتيب حسب قوة MACD
            if candidates:
                state["data_source"]=f"MACD {len(candidates)} مولعة سيولة عالية"
                return candidates
    except Exception as e:
        state["binance_status"]=f"خطأ: {str(e)[:20]}"
    return []
...
def engine():
    while True:
        try:
            ...
            # 1. تحديث المراكز المفتوحة
            for p in state["positions"]:
                try:
                    r=requests.get(f"https://api.binance.com/api/v3/ticker/price?symbol={p[0]}USDT", timeout=3)
                    if r.status_code==200:
                        np=float(r.json()["price"])
                        p[3]=np; pp=(p[3]-p[2])/p[2]*100; p[4]=round(config["per_trade"]*pp/100,3); p[5]=round(pp,2)
                        p[6]=f"MACD مولعة {pp:.1f}%" if pp>0 else f"تراجع {pp:.1f}%"
                except: pass
            ...
            # 2. فحص صافي الربح - نقفل الكل؟
            total_net = state["ghair"] + state["safi"]
            target_value = config["capital"] * config["target_pct"] / 100
            if state["ghair"]>0 and state["ghair"] >= target_value and len(state["positions"])>0:
                # نقفل الكل ونحول للصافي
                profit = state["ghair"]
                state["safi"]+=profit
                state["fixed"]=config["capital"]+state["safi"]
                state["trades_closed"]+=len(state["positions"])
                state["positions"]=[]
                state["ghair"]=0.0
                state["last"]=f"قفل صافي ربح {profit:.2f}$ = {config['target_pct']}%"

            # 3. فحص الخسائر - حول للمستشفى
            to_hospital=[]
            for p in state["positions"]:
                if p[5] <= -config["sl_pct"]: # خسرت أكثر من 0.8%
                    to_hospital.append(p)
            for p in to_hospital:
                state["positions"].remove(p)
                # حول للمستشفى
                state["treatment"].append([p[0],"علاج",p[2],p[3],0.0,0.0,0.35,f"{p[0]} يعالج {p[4]:.2f}$",abs(p[4]),config["per_trade"]*0.01])

            # 4. دخول جديد بـ MACD إذا فيه مكان
            if len(state["positions"]) < config["max_pos"] and not state["specialty"]:
                movers=get_movers_macd(10)
                existing=set([p[0] for p in state["positions"]+state["treatment"]])
                for sym,pct,price,vol,hist in movers:
                    if sym not in existing and len(state["positions"])<config["max_pos"]:
                        state["positions"].append([sym,"SPOT",price*0.999,price,0.0,0.0,f"MACD {pct:.1f}% Vol {vol/1000000:.1f}M",time.time()])
                        existing.add(sym)

            # تخصصي؟
            if len(state["treatment"])>=config["hospital_cap"]:
                state["specialty"]=True; state["is_running"]=False
            elif state["specialty"] and len(state["treatment"])<3:
                state["specialty"]=False; state["is_running"]=True

            time.sleep(4) # كل 4 ثواني عشان ما نضغط Binance
        except Exception as e:
            time.sleep(2)
...
def control(cmd):
    if cmd=="toggle":
        ...
        if state["is_running"] and len(state["positions"])==0:
            state["last"]="تشغيل - يبحث MACD..."
    elif cmd=="lock":
        profit=sum(p[4] for p in state["positions"] if p[4]>0)
        state["safi"]+=profit; state["positions"]=[]; state["ghair"]=0.0
    elif cmd=="reset":
        state["fixed"]=config["capital"]; state["safi"]=0.0; state["ghair"]=0.0; state["loss_pool"]=0.0; state["trades_closed"]=0
        state["positions"]=[]; state["treatment"]=[]; state["doctor"]={"healed":0,"profit":0.0,"start":time.time(),"rate":95.4}
        state["specialty"]=False; state["is_running"]=True; state["last"]="تصفير - يبدأ MACD تلقائي"
    elif cmd=="try":
        movers=get_movers_macd(3)
        if movers: s=movers[0]; state["positions"].append([s[0],"SPOT",s[2]*0.999,s[2],0.0,0.0,f"MACD تجربة {s[1]:.1f}%",time.time()])
    elif cmd=="specialty":
        state["specialty"]=not state["specialty"]; state["is_running"]=not state["specialty"]
...
@app.route('/api/data')
def api_data():
    total=state["fixed"]+state["safi"]+state["ghair"]-state["loss_pool"]; elapsed=int(time.time()-state["doctor"]["start"]); total_cases=state["doctor"]["healed"]+len(state["treatment"]); rate=round(state["doctor"]["healed"]/total_cases*100 if total_cases else 95.4,1)
...
<div class="docbar" id="docbar"><span id="elapsed">⏱️ 0ث</span><span id="rate">📊 شفاء: 95.4%</span><span id="profit">💰 علاج: +0.00$</span><span id="healed">شفى: 0</span><span id="spec">🏥 طبيعي MACD</span></div>
<div class="panel"><h3>⚙️ تحكم - MACD + سيولة + صافي ربح % من رأس المال</h3><div class="ctrls"><div class="ctrl"><label>💰 رأس المال $</label><input id="cap" type="number" value="1000" onchange="save()"></div><div class="ctrl"><label>📦 حجم الصفقة $</label><input id="per" type="number" value="100" onchange="save()"></div><div class="ctrl"><label>🎯 صافي ربح %</label><input id="targ" type="number" value="0.5" step="0.1" onchange="save()"><div style="font-size:9px;color:#ffeb3b" id="targVal">0.5% = 5$ قفل الكل</div></div><div class="ctrl"><label>🏥 سعة المستشفى</label><input id="hcap" type="number" value="15" onchange="save()"></div></div></div>
<div class="header"><button class="hbtn orange" onclick="ctrl('try')">🧪 دخول MACD يدوي</button><button class="hbtn yellow" onclick="ctrl('reset')">🔄 تصفير AUTO</button><button class="hbtn red" onclick="ctrl('lock')">🔒 قفل صافي الربح</button><button class="hbtn green" id="btnRun" onclick="ctrl('toggle')">⏸️ ايقاف</button><button class="hbtn blue" id="btnSpec" onclick="ctrl('specialty')">🏥 تخصصي</button></div>
<div class="cards"><div class="c"><div class="l">💰 ثابت</div><div class="v w" id="f1">1000$</div></div><div class="c ph"><div class="l">📦 الصيدلية</div><div class="v r" id="f2">-0.00$</div><div class="l" id="f2c">0 دواء</div></div><div class="c"><div class="l">💹 صافي محقق</div><div class="v g" id="f3">+0$</div></div><div class="c gold"><div class="l">💎 الإجمالي</div><div class="v g" id="f5">1000$</div></div><div class="c"><div class="l">📈 غير محققة - مجموع</div><div class="v g" id="f6">+0.00$</div></div></div>
