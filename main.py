...
def try_add_fast():
    ...
    try:
        ...
        for d in sorted_data:
            ...
            side="LONG" if ch>=0 else "SHORT" if state["strategy"]==1 else None
            if state["strategy"]>=2:
    ...
    while True:
        try:
            ...
            if state["paused_until"]>0 and time.time()>=state["paused_until"]:
                ...
                # الإصلاح المهم: تصفير القمة للربح الجديد المؤمن عشان ما يرجع يوقف فوراً
                state["peak_profit"]=state["realized"]+state["unrealized"]
                print(f"✅ انتهى الإيقاف - القمة الجديدة {state['peak_profit']:.2f}$")
            ...
            if state["paused_until"]==0 and state["peak_profit"]>0:
                ...
                if drop >= DROP_LIMIT:
                    ...
                    # الإصلاح الجوهري: القمة تصير = الربح المؤمن الجديد
                    state["peak_profit"]=state["realized"]
                    recalc_balance()
                    print(f"🛑 حماية 3$ - قفل الكل وتأمين {state['realized']:.2f}$")
            ...
            print("worker",e); time.sleep(1)
        time.sleep(0.5)

threading.Thread(target=worker,daemon=True).start()

HTML="""<!DOCTYPE html><html dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>V82 FIXED</title><link href="https://fonts.googleapis.com/css2?family=Cairo:wght@700;900&family=JetBrains+Mono:wght@800&display=swap" rel="stylesheet"><style>
...
.glass{background:linear-gradient(145deg,rgba(38,42,90,0.9),rgba(21,24,51,0.95));border:1.5px solid rgba(255,255,255,0.12);border-radius:20px;padding:14px;backdrop-filter:blur(12px)}
input,select{background:#07091e;color:#ffcc00;border:2.5px solid #ffcc00aa;border-radius:12px;padding:11px 10px;font-size:18px;font-weight:900;text-align:center;direction:ltr;min-width:90px}
...
.cardx.profit{border-color:#00ff88;box-shadow:0 0 35px #00ff8855}.cardx.loss{border-color:#ff3d57;box-shadow:0 0 35px #ff3d5755}
.lbl{color:#9aa0c5;font-size:10.5px;font-weight:800}.val{font-size:18.5px;font-weight:900;margin-top:6px}
...
th{background:linear-gradient(145deg,#2e3268,#23264e);color:#00ff88;padding:12px 6px;font-size:12px;font-weight:900}
...
<div class="header">👑 V82 TURBO FIXED</div>
<div class="sub">إصلاح الحلقة - القمة تتصفر بعد الحماية - يرجع لحاله بعد 15 دقيقة</div>
<div id="protectBox" class="protect">🛑 حماية مفعلة</div>
<div class="glass" style="margin-bottom:14px;text-align:center;border:1.5px solid #00ff88aa">
<div style="color:#00ff88;font-weight:900;margin-bottom:8px;font-size:13px">🎮 استراتيجيات + حماية 3$</div>
...
<span style="background:linear-gradient(145deg,#00ff88,#00c853);color:#000;padding:8px 14px;border-radius:12px;font-weight:900;font-size:12px">نشط: {{strategies[s.strategy]}} | قمة: {{'%.2f'|format(s.peak_profit)}}$ | حماية: {{s.protection_hits}}</span>
</div>
</div>
<div class="dashboard">
<div class="cardx"><div class="lbl">💰 ثابت</div><div class="val" id="total"><span class="ltr">{{'%.0f'|format(s.total_capital)}}$</span></div></div>
<div class="cardx"><div class="lbl">🔓 حر</div><div class="val" id="bal"><span class="ltr">{{'%.0f'|format(s.balance)}}$</span></div></div>
<div class="cardx {{'profit' if s.realized>=0 else 'loss'}}" id="realCard"><div class="lbl">💵 صافي</div><div class="val" id="real" style="color:{{'#00ff88' if s.realized>=0 else '#ff3d57'}}"><span class="ltr">{{'%+.2f'|format(s.realized)}}$</span></div></div>
<div class="cardx {{'profit' if s.unrealized>=0 else 'loss'}}" id="unrealCard"><div class="lbl">📈 غير محققة</div><div class="val" id="unreal"><span class="ltr">{{'%+.2f'|format(s.unrealized)}}$</span></div></div>
<div class="cardx"><div class="lbl">💎 إجمالي | قمة</div><div class="val" id="equity"><span class="ltr">{{'%.2f'|format(s.total_capital + s.realized + s.unrealized)}}$</span><br><span class="ltr" style="font-size:12px;color:#ffcc00" id="peak">قمة: {{'%.2f'|format(s.peak_profit)}}$</span></div></div>
...
<button class="btn btn-gold" onclick="fetch('/force_resume').then(()=>location.reload())">🚀 فك التعليق وإعادة التشغيل الآن</button>
</div>
...
function refresh(){
 fetch('/api').then(r=>r.json()).then(d=>{
  ...
  let longs=d.trades.filter(t=>t.side=='LONG').length;document.getElementById('ls').innerHTML=`<span class="ltr">${longs}/${d.trades.length-longs} | ${d.cycles} | 🛡️${d.protection_hits}</span>`;
  ...
  if(d.paused_until > Date.now()/1000){
    ...
    box.innerText=`🛑 حماية 3$ - القمة ${d.peak_profit.toFixed(2)}$ - تم تأمين الربح - الرجوع بعد ${Math.floor(left/60)}د ${left%60}ث`;
  }else{ box.className='protect'; box.innerText='';}
 });
}
