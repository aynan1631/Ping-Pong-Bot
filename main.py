...
STATE = {"is_running": False,"total_capital": 76.44,"trade_value": 10.00,"profit_target_cents": 10,"max_trades": 2,"realized": 0.00,"unrealized": 0.00,"patients": [],"coins": [],"history": [],"log": "📜 سجل الصفقات جاهز"}
    ...
    while True:
        try:
            ...
            if unreal>=target and STATE["patients"]:
                # نسجل الصفقة في السجل قبل البيع
                ...
                STATE["patients"]=[]; STATE["log"]=f"✅ باع وحقق {target:.2f}$ - تسجل في السجل"
                continue
...
.btn{padding:16px 32px;border-radius:16px;border:none;font-weight:900;font-size:18px;cursor:pointer}
.btn-start{background:linear-gradient(135deg,#22c55e,#16a34a);color:#fff}.btn-stop{background:linear-gradient(135deg,#ef4444,#dc2626);color:#fff}
...
.big-num{font-size:64px;font-weight:900;background:linear-gradient(90deg,#facc15,#fde047);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
...
.mini b{display:block;font-size:14px;color:#cbd5e1;margin-bottom:8px}
.mini.num{font-size:26px}
...
.input-gold{border-color:#facc15;background:rgba(250,204,21,0.15);font-size:26px}
...
.history-item b{font-size:18px;color:#4ade80}
.log{font-size:15px;color:#e2e8f0;margin-top:12px;padding:12px;background:rgba(0,0,0,0.4);border-radius:12px;border:2px solid rgba(255,255,255,0.08);font-weight:700}
</style></head><body>
<div class="header"><div class="logo">👑 مستشفى ملكي كامل | صفقات + طبيب + مستشفى + سجل | <span class="num" id="cap2" style="-webkit-text-fill-color:#facc15;font-size:26px">76.44</span></div><div style="display:flex;gap:12px"><button class="btn btn-start" onclick="ctrl('start')">▶ تشغيل ملكي</button><button class="btn btn-stop" onclick="ctrl('stop')">⏹ إيقاف</button></div></div>
...
<div class="card"><div class="title">⚙️ إعداداتك</div><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px"><div><div class="title">💰 قيمة الصفقة</div><input class="input input-gold num" id="tv" value="10.00" onchange="save()"></div><div><div class="title">🎯 هدف سنت</div><input class="input input-gold num" id="cents" value="10" onchange="save()"></div><div><div class="title">🛏️ عدد المرضى</div><input class="input num" id="mt" value="2" onchange="save()"></div></div><div style="text-align:center;margin-top:20px"><div class="title" style="font-size:18px">الهدف الإجمالي</div><div class="dollar"><span class="num" id="dollars">0.10</span> $</div></div></div>
</div>

<div class="grid">
<div class="card"><div class="title" style="font-size:20px">💎 العملات القوية الحلال</div><div id="coins"></div></div>
<div class="card card-history"><div class="title" style="font-size:20px">📜 مكان الصفقات المقفلة - سجل الأرباح</div><div style="display:flex;gap:10px;margin-bottom:12px"><div class="mini"><b>💰 عدد الصفقات المقفلة</b><span class="num" id="h-count" style="color:#4ade80;font-size:28px">0</span></div><div class="mini"><b>✅ إجمالي محقق</b><span class="num" id="h-total" style="color:#facc15;font-size:28px">0.00$</span></div></div><div id="history" style="max-height:320px;overflow-y:auto"></div></div>
</div>

<div class="grid3">
<div class="card"><div class="title">💎 غرفة عادية - ربحان</div><div id="room-normal"></div></div>
...
document.getElementById('state').innerText=j.is_running?'👑 شغال ملكي - كامل مع سجل الصفقات':'⏹ متوقف كامل';
...
let ch='';j.coins.forEach(c=>{ch+=`<div class=coin><div><b>${c.symbol}</b><div class=num style='color:#94a3b8;font-size:15px'>${(c.vol/1000000).toFixed(1)}M</div></div><span style='background:#16a34a;color:#fff;padding:8px 14px;border-radius:20px;font-size:13px;font-weight:900'>MACD صاعد</span></div>`});
document.getElementById('coins').innerHTML=ch;

// سجل الصفقات المقفلة
let hh='';if(j.history.length==0) hh='<div style=text-align:center;padding:30px;color:#64748b;font-size:16px;font-weight:800>📜<br>لا يوجد صفقات مقفلة بعد<br><span style=font-size:12px>بتظهر هنا أول ما تربح</span></div>';
else j.history.forEach(h=>{hh+=`<div class=history-item><div><b>${h.symbol}</b><div class=num style="font-size:12px;color:#94a3b8">${h.time} | دخول ${f(h.entry)} → خروج ${f(h.exit)}</div></div><div style=text-align:left><div class=num style="color:#4ade80;font-size:18px;font-weight:900">+${f(h.pnl)}$</div><div class=num style="color:#4ade80;font-size:13px">+${f(h.percent)}%</div></div></div>`;});
document.getElementById('history').innerHTML=hh;

let normal=j.patients.filter(p=>p.pnl_percent > -3);
let hosp=j.patients.filter(p=>p.pnl_percent <= -3 && p.pnl_percent > -6);
let doc=j.patients.filter(p=>p.pnl_percent <= -6);
function render(list){
if(list.length==0) return '<div style=text-align:center;padding:20px;color:#64748b;font-size:16px;font-weight:800>فارغ ✅</div>';
let h='';list.forEach(p=>{
let cls=p.pnl_percent<=-6?'doctor':p.pnl_percent<=-3?'hospital':'normal';
h+=`<div class="patient ${cls}"><div><b>${p.symbol}</b><div style="font-size:13px;color:#cbd5e1">${p.status}</div><div class=num style="font-size:13px;color:#94a3b8">دخول ${p.entry}</div></div><div style=text-align:left><div class="pnl num" style="color:${p.pnl>=0?'#4ade80':'#f87171'}">${f(p.pnl)}$</div><div class="perc num" style="color:${p.pnl_percent>=0?'#4ade80':'#f87171'}">${p.pnl_percent>=0?'+':''}${f(p.pnl_percent)}%</div></div></div>`;
});return h;
}
