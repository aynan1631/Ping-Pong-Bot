...
def dash():
 return """
<html dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>V33.5 LUXURY</title>
<style>
body{background:radial-gradient(circle at top,#1e293b 0%,#020617 100%);color:#fff;font-family:Tahoma,Arial;padding:12px}
.card{background:linear-gradient(135deg,rgba(15,23,42,0.9),rgba(30,41,59,0.8));backdrop-filter:blur(12px);border:1px solid rgba(251,191,36,0.25);border-radius:16px;padding:12px;text-align:center;margin:5px;box-shadow:0 0 20px rgba(0,0,0,0.5)}
table{width:100%;background:rgba(15,23,42,0.9);border:1px solid rgba(251,191,36,0.25);border-radius:18px;border-collapse:collapse;overflow:hidden;box-shadow:0 0 35px rgba(0,0,0,0.7)}
...
.green{color:#22c55e;font-weight:bold;text-shadow:0 0 8px rgba(34,197,94,0.4)}.red{color:#ef4444;font-weight:bold}
.badge{padding:4px 10px;border-radius:20px;font-size:11px;font-weight:bold}.short{background:linear-gradient(90deg,#7f1d1d,#ef4444);color:#fff;box-shadow:0 0 10px rgba(239,68,68,0.3)}.long{background:linear-gradient(90deg,#064e3b,#22c55e);color:#fff}
.capital{color:#fde68a;font-weight:900;text-shadow:0 0 10px rgba(251,191,36,0.6);background:linear-gradient(90deg,rgba(251,191,36,0.22),rgba(251,191,36,0.05));border:1px solid rgba(251,191,36,0.2);border-radius:10px;padding:6px 10px}
.btn{border:0;padding:8px 14px;border-radius:10px;color:#fff;cursor:pointer;margin:2px;font-weight:bold}
</style></head><body>
<h2 style="text-align:center;color:#fbbf24;text-shadow:0 0 15px rgba(251,191,36,0.6)">💎 V33.5 LUXURY - فلتر EMA200 المزدوج (10 صفقات إجباري)</h2>
<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px"><div class="card"><small>رأس المال</small><br><b id="cap"></b></div><div class="card"><small>المحقق</small><br><b id="real" class="green"></b></div><div class="card"><small>العائم</small><br><b id="float"></b></div><div class="card"><small>الكلي</small><br><b id="total"></b></div></div>
<div class="card"><div id="trend" style="color:#fbbf24"></div><br><b>🎯</b><input id="tin" type="number" style="width:70px;border-radius:8px;padding:4px"><button class="btn" style="background:#6c5ce7" onclick="setT()">حفظ</button><button class="btn" style="background:#ff3b3b" onclick="closeAll()">🔒 قفل الكل</button></div>
<div style="overflow:auto">
<table><thead><tr><th>العملة</th><th>الجانب</th><th>💰 رأس المال</th><th>دخول</th><th>حالي</th><th>$</th><th>%</th><th>تقلب</th></tr></thead><tbody id="tb"></tbody></table>
</div>
<script>
...
def sc():
 bot_state["base_capital"]=float(request.json.get("capital",5000)); return jsonify({"ok":True})
