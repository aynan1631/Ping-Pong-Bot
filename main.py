...
HTML="""<!DOCTYPE html><html dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>V75 LUXURY DUAL</title><link href="https://fonts.googleapis.com/css2?family=Cairo:wght@700;900&family=JetBrains+Mono:wght@800&display=swap" rel="stylesheet"><style>
...
.strat-glass{border:1.5px solid #00ff88aa;box-shadow:0 0 25px #00ff8822}
</style></head><body>
<div class="header">👑 V73 LUXURY FINAL — V75 DUAL</div>
<div class="sub">محرك V73 الذهبي + واجهة V74 الفخمة | TP 0.35$ | SL -0.65$ | الربح محفوظ عند التبديل</div>
<div class="glass strat-glass" style="margin-bottom:14px;text-align:center">
...
function refresh(){
 fetch('/api').then(r=>r.json()).then(d=>{
  ...
  let realCard=document.getElementById('realCard');let realEl=document.getElementById('real');
  ...
  let unrealCard=document.getElementById('unrealCard');let unrealEl=document.getElementById('unreal');
...
def close_all():
    for t in state["trades"]: cooldown[t["sym"]]=time.time()
    state["realized"]=round(state["realized"]+state["unrealized"],2); state["trades"]=[]; state["unrealized"]=0; state["cycles"]+=1; state["balance"]=state["total_capital"]
    return "OK"
@app.route('/close_one')
def close_one():
    ...
    for t in list(state["trades"]):
        if t["s"]==sname:
            state["realized"]=round(state["realized"]+t["pnl"],2); cooldown[t["sym"]]=time.time(); state["trades"].remove(t); break
