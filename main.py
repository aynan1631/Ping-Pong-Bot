...
def get_btc_all_tf():
    try:
        ...
        return {"4h":"UP","1h":"UP","15m":"UP","5m":"UP"}

def is_mtf_aligned(sym, side, btc_trends):
    try:
        # فحص العملة: 5m + 15m + 1h + 4h لازم كلهم نفس جانب الصفقة
        for tf in ["5m","15m","1h","4h"]:
            t=get_trend_tf(sym, tf)
            if t is None: continue
            if side=="LONG" and t!="UP": return False
            if side=="SHORT" and t!="DOWN": return False
        # فحص BTC وين رايح
        if side=="LONG":
            if btc_trends.get("4h")=="DOWN" or btc_trends.get("1h")=="DOWN": return False
        if side=="SHORT":
            if btc_trends.get("4h")=="UP" or btc_trends.get("1h")=="UP": return False
        return True
    except:
        return True
...
def try_add_one():
    ...
    btc_trends=get_btc_all_tf() # اتجاه BTC في كل الفريمات
    ...
    for sym in get_volatile_coins(120):
        ...
        # === الفلتر الجديد اللي طلبته: كل الفريمات + BTC ===
        if not is_mtf_aligned(sym, side, btc_trends):
            continue
...
def worker():
    time.sleep(5) # يمنع Application failed to respond
    while True:
        try:
            per=state["per_trade"]; max_open=int(state["total_capital"]//per) if per>0 else 10
