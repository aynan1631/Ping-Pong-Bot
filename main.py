...
state={"total_capital":5000.0,"per_trade":500.0,"balance":5000.0,"realized":0.0,"unrealized":0.0,"total_target":5.0,"trades":[],"TP":5.0,"SL":-15.0,"compound":True,"cycles":0}
        ...
        try:
            ...
            if state["trades"]:
                ...
                if state["total_target"]>0.1 and state["unrealized"]>=state["total_target"]:
                    # قفل الكل
                    equity = state["balance"] + sum(t["cap"]+t["pnl"] for t in state["trades"])
                    for t in list(state["trades"]):
                    ...
                    # تراكم
                    if state["compound"]:
                        state["total_capital"]=round(equity,2)
                        state["balance"]=round(equity,2)
                        # يكبر الصفقة تلقائي كل 10 دورات
                        state["per_trade"]=round(state["total_capital"]/10,2)
                    else:
                        state["balance"]=round(equity,2)
                    time.sleep(0.5)
...
<title>V57 COMPOUND TURBO</title>
...
.ctrl{background:linear-gradient(145deg,#262a5a,#151833);border:2px solid #ffcc0055;border-radius:18px;padding:12px;text-align:center}
...
.cardx{background:linear-gradient(145deg,#2a2e6a,#1a1c3f);border:2px solid #ffffff20;border-radius:20px;padding:14px;text-align:center}
.cardx.gold{border-color:#ffcc00aa;box-shadow:0 0 25px #ffcc0033}
.cardx.compound{border-color:#00ff88aa;box-shadow:0 0 25px #00ff8833;animation:pulse 2s infinite}
@keyframes pulse{0%{box-shadow:0 0 15px #00ff8833}50%{box-shadow:0 0 30px #00ff8866}100%{box-shadow:0 0 15px #00ff8833}}
...
.table-wrap{background:linear-gradient(145deg,#1e2147,#151833);border-radius:20px;overflow:hidden;border:2px solid #ffffff18}
...
.badge-long{background:linear-gradient(145deg,#00e676,#00c853);color:#000;padding:8px 18px!important;border-radius:25px;font-size:14px!important;font-weight:900!important;min-width:75px;display:inline-block}
.badge-short{background:linear-gradient(145deg,#ff1744,#d50000);color:#fff;padding:8px 18px!important;border-radius:25px;font-size:14px!important;font-weight:900!important;min-width:75px;display:inline-block}
...
<div class="header">💎 V57 COMPOUND TURBO 🚀 - رأس المال يكبر تلقائي</div>
<div class="sub">⚡ دورة رقم {{s.cycles}} - كل +5$ يضاف لرأس المال ويكبر اللوت</div>
<div class="top-controls">
  <div class="ctrl"><div class="ctrl-title">💰 إجمالي رأس المال (يكبر تلقائي)</div><div style="display:flex;gap:8px;justify-content:center;align-items:center;flex-wrap:wrap">
  ...
  <div class="ctrl"><div class="ctrl-title">📦 رأس مال الصفقة (تلقائي)</div><div style="display:flex;gap:8px;justify-content:center;align-items:center;flex-wrap:wrap">
...
<div class="dashboard">
  <div class="cardx gold"><div class="lbl">💰 رصيد حر (أرباح)</div><div class="val" id="bal" style="color:#ffcc00"><span class="ltr">{{'%.2f'|format(s.balance)}}$</span></div></div>
  ...
  <div class="cardx compound"><div class="lbl">💵 الكلي يكبر 🚀</div><div class="val" id="total"><span class="ltr">{{'%.2f'|format(s.total_capital)}}$</span></div></div>
  ...
  <div class="cardx"><div class="lbl">✅ محقق تراكمي</div><div class="val" id="real"><span class="ltr">{{'%+.2f'|format(s.realized)}}$</span></div></div>
  <div class="cardx gold"><div class="lbl">⚖️ LONG / SHORT | دورات</div><div class="val" id="ls" style="font-size:18px!important"><span class="ltr">{{s.trades|selectattr('side','equalto','LONG')|list|length}} / {{s.trades|selectattr('side','equalto','SHORT')|list|length}} | {{s.cycles}}🔄</span></div></div>
</div>
...
function refresh(){
 fetch('/api').then(r=>r.json()).then(d=>{
  ...
  document.getElementById('total').innerHTML=`<span class="ltr">${d.total_capital.toFixed(2)}$</span>`;
...
@app.route('/set_total')
def set_total():
    try: v=float(request.args.get('v')); state["total_capital"]=v; state["balance"]=v
    except: pass
    return "OK"
@app.route('/set_per')
def set_per():
...
def set_tar():
    ...
    return "OK"
@app.route('/reset')
def reset(): state["total_capital"]=5000; state["balance"]=5000; state["per_trade"]=500; state["realized"]=0; state["unrealized"]=0; state["trades"]=[]; state["cycles"]=0; cooldown.clear(); return "OK"
@app.route('/close_all')
def close_all():
    equity = state["balance"] + sum(t["cap"]+t["pnl"] for t in state["trades"])
    for t in list(state["trades"]): cooldown[t["sym"]]=time.time()
    state["trades"]=[]; state["realized"]=round(state["realized"]+state["unrealized"],2); state["unrealized"]=0
    state["total_capital"]=round(equity,2); state["balance"]=round(equity,2)
    return "OK"
@app.route('/close_one')
def close_one():
