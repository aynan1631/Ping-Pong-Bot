...
def get_volatile_coins(limit=100):
    try:
        ...
        for d in data:
            ...
            if sym in cooldown and time.time()-cooldown[sym]<30: continue # كان 240 ثانية - صار 30
            cands.append((sym,abs(ch)))
        cands.sort(key=lambda x: x[1], reverse=True)
        return [s for s,_ in cands[:limit]]
    except: return ["HEMIUSDT","PENGUUSDT","TUTUSDT","TRUMPUSDT","ASTERUSDT"]
...
def get_btc_trend():
    try:
        d=requests.get(f"https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=1h&limit=50",timeout=2).json()
...
def get_btc_all_tf():
    try:
        if "BTC_ALL" in trends_cache and time.time() - trends_cache["BTC_ALL"]['time'] < 30:
            return trends_cache["BTC_ALL"]['data']
        data = {tf: get_trend_tf("BTCUSDT", tf) for tf in ["1h","4h"]}
        trends_cache["BTC_ALL"] = {'data': data, 'time': time.time()}
        return data
    except: return {"4h":"UP","1h":"UP"}

def is_mtf_aligned(sym, side, btc_trends):
    try:
        t1h = get_trend_tf(sym, "1h")
        t4h = get_trend_tf(sym, "4h")
        if t1h and t4h:
            if side=="LONG" and (t1h!="UP" or t4h!="UP"): return False
            if side=="SHORT" and (t1h!="DOWN" or t4h!="DOWN"): return False
        if side=="LONG" and btc_trends.get("4h")=="DOWN": return False
        if side=="SHORT" and btc_trends.get("4h")=="UP": return False
        return True
    except: return True
...
def try_add_one():
    ...
    for sym in get_volatile_coins(100):
        ...
        side=None
        if d["price"]>d["ema"] and d["price"]>d["sma50"] and d["trend4H"]=="UP" and 40<d["rsi"]<66:
            side="LONG"
        elif d["price"]<d["ema"] and d["price"]<d["sma50"] and d["trend4H"]=="DOWN" and 34<d["rsi"]<60:
            side="SHORT"
    ...
    while True:
        try:
            ...
            if state["trades"]:
                ...
                # لو نايم أكثر من دقيقة - امسح الكولداون غصب
