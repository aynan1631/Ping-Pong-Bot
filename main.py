"""
V100 LEGEND REAL - نفس لوحتك بالضبط + تداول حقيقي في Binance
- اذا وصل ثابت 21$ يظهر في Binance نفسه ✅
"""
    ...
    try:
        ...
        try:
            ...
            if isinstance(funding, list) and len(funding)>0:
                total += float(funding[0].get('free',0)) + float(funding[0].get('locked',0))
        except: pass
        if total < 0.5:
            try:
...
def real_buy(sym, usdt):
    if not REAL_CLIENT or state["mode"]!="REAL":
        state["binance_status"]="⛔ API ناقص"; return None
    try:
        usdt = max(float(usdt), 5.0)
        bal = get_real_total_balance()
        if bal < usdt:
            state["binance_status"]=f"❌ رصيد الفوري {bal:.2f}$ ناقص - حول من التمويل للفوري"
            return None
        ...
        state["binance_status"]=f"✅ شراء حقيقي {sym} {qty:.3f} @ {price:.4f}"
        return {"price":price,"qty":qty}
    except BinanceAPIException as e:
        print(f"BUY FAIL {e}"); state["binance_status"]=f"❌ شراء {sym} فشل: {e.message[:50]}"
        if "Insufficient" in e.message: state["binance_status"]="❌ الفوري فاضي - حول USDT من التمويل للفوري في Binance"
        return None
    except Exception as e:
        print(f"BUY ERR {e}"); state["binance_status"]=f"❌ خطأ {str(e)[:50]}"; return None

def real_sell(sym, qty):
    if not REAL_CLIENT: return False
    try:
        step,prec=get_prec(sym); qty=math.floor(qty/step)*step
...
def get_tradingview_doctors(exclude=[], limit=2):
    candidates=[]
    try:
...
def update_balances():
    try:
        if REAL_CLIENT:
            ...
            if state["mode"]=="REAL": config["capital"]=total; state["fixed"]=total+state["safi"]
    except: pass
...
def engine():
    while True:
        try:
            ...
            config["per_trade"] = max(get_dynamic_per_trade(),5.0)
            ...
            for p in state["positions"]:
                try:
                    ...
                    if r.status_code==200:
                        ...
                        if len(p)>=9: p[4]=round((p[3]-p[2])*p[8],4)
                        else: p[4]=round(config["per_trade"]*pp/100,3)
                ...
                for strongest in strongest_list:
                    ...
                    if sym not in exist:
                        ...
                        if buy_res:
                            ...
                            exist.add(sym); used.add(target[0])
            ...
            for d in state["doctor_positions"][:]:
                try:
                    ...
                    if r.status_code==200:
                        ...
                        invoice=d[8]; target_p=invoice*0.60; alive=time.time()-d[7]
            ...
            if state["ghair"] >= config["target_dollar"] and len(state["positions"])>0 and not state["healing_mode"]:
                ...
                state["binance_status"]=f"✅ قفل ربح حقيقي {profit:.3f}$ - رصيد Binance {state['fixed']:.2f}$"
...
def reset_trading_state(new_capital):
    state["positions"]=[]; state["treatment"]=[]; state["doctor_positions"]=[]; state["safi"]=0.0; state["max_safi"]=0.0; state["ghair"]=0.0; state["loss_pool"]=0.0; state["trades_closed"]=0; state["fixed"]=new_capital; state["doctor"]={"healed":0,"profit":0.0,"start":time.time(),"rate":99.5}; state["healing_mode"]=False

@app.route('/api/control/<cmd>')
def control(cmd):
    if cmd=="toggle":
        state["is_running"]=not state["is_running"]
        if state["is_running"]: state["binance_status"]="▶️ يشتغل REAL - يحاول يشتري حقيقي"
    ...
    elif cmd=="reset": reset_trading_state(config["capital"]); state["is_running"]=True
    elif cmd=="ban_clean":
        for p in state["treatment"][:]:
        ...
        state["mode"]="TESTNET"; config["capital"]=10000.0; reset_trading_state(10000.0); state["test_balance"]="10000.00"; state["is_running"]=True
        state["binance_status"]=f"TESTNET {config['capital']:.2f}$"
        return jsonify({"ok":True, "capital": config["capital"], "mode": "TESTNET"})
    elif cmd=="mode_real":
        state["mode"]="REAL"; total = get_real_total_balance(); state["real_balance"]=f"{total:.2f}"; config["capital"]=total; reset_trading_state(total); state["is_running"]=True
    ...
    return jsonify({"ok":True, "capital": config["capital"], "mode": state["mode"], "real_balance": state["real_balance"], "is_running":state["is_running"]})
...
<div class="top"><span id="elapsed">0s</span><span id="rate">V100 REAL - 0/30</span><span id="profit">+0.00$</span><span id="healed">0</span><span id="spec">REAL</span></div>
<div class="mode-bar">
...
<button id="btnReal" class="mode-btn" onclick="switchMode('mode_real')" style="background:#D4AF37;color:#000;border-color:#D4AF37">👑 حقيقي LEGEND REAL<br><span id="realBal" style="font-size:11px">20.52</span></button>
</div>
<div class="panel"><h3 id="mainTitle">👑 V100 LEGEND - رصيدك 20.52$ حقيقي 👑</h3><div class="grid">
<div class="box" style="border:2px solid #FFD700"><label>💰 راس المال $ 👑 حقيقي REAL</label><div class="step-row"><button class="step-btn" type="button" onclick="stepCap(-1)">-</button><input id="cap" type="text" inputmode="decimal" lang="en" value="20.52"><button class="step-btn" type="button" onclick="stepCap(1)">+</button></div><div id="capInfo" class="small-info">👑 20.52$ BINANCE REAL</div></div>
...
<div class="foot"><span id="src">V100 REAL</span><span id="bin">👑 جاهز REAL</span><span id="time">...</span></div>
<script>
function enforceEnglish(el){ let v=el.value||""; const ar='٠١٢٣٤٥٦٧٨٩'; const en='0123456789'; for(let i=0;i<10;i++){ v=v.split(ar[i]).join(en[i]); } v=v.replace(/[^0-9.]/g,''); let parts=v.split('.'); if(parts.length>2){ v=parts[0]+'.'+parts.slice(1).join(''); } el.value=v; }
...
async function ctrl(c){ const res = await fetch('/api/control/'+c); const j = await res.json(); if(j.is_running!==undefined){ let btn=document.getElementById('btnRun'); if(j.is_running){ btn.innerText='⏸️ ايقاف REAL'; btn.style.background='#FF3344'; } else { btn.innerText='▶️ تشغيل حقيقي REAL'; btn.style.background='#10B981'; } } load(); }
...
async function load(){
  try{
    ...
    document.getElementById('capInfo').innerText='👑 '+d.real_balance+'$ BINANCE REAL = '+en(d.config.capital,2)+'$';
    document.getElementById('f1').innerText=en(d.fixed,2)+'$'; document.getElementById('f2').innerText=en(d.loss_pool,2)+'$'; document.getElementById('f2c').innerText=en(d.treatment.length,0)+' دواء'; document.getElementById('f3').innerHTML='<span class="'+(Number(d.safi)>=0?'profit-pos':'profit-neg')+'">+'+en(d.safi,3)+'$</span>'; document.getElementById('f5').innerText=en(d.fixed+d.ghair-d.loss_pool,2)+'$'; document.getElementById('f6').innerText=en(d.ghair,3)+'$'; document.getElementById('bin').innerText=d.binance_status; document.getElementById('src').innerText=d.data_source; document.getElementById('time').innerText=new Date().toLocaleTimeString('en-GB',{hour12:false}); document.getElementById('testBal').innerText=d.test_balance+' USDT'; document.getElementById('realBal').innerText=d.real_balance+' USDT'; document.getElementById('mainTitle').innerText='👑 V100 LEGEND REAL - رصيدك '+d.real_balance+'$ حقيقي في Binance 👑';
    let h=''; for(const p of d.positions){ let cls=Number(p[5])>=0?'profit-pos':'profit-neg'; let qtyInfo = p.length>=9? en(p[8],3)+' '+p[0] : ''; h+=`<div class="rw" style="grid-template-columns:1.2fr 0.8fr 1.2fr 0.8fr 0.8fr 0.8fr 0.6fr"><div>${p[0]}<div style="font-size:9px;color:#FFD700">${qtyInfo}</div></div><div><span class="badge" style="background:#10B981;color:#000">REAL</span></div><div class="${cls}">${p[6]}</div><div>${en(p[2],4)}</div><div>${en(p[3],4)}</div><div class="${cls}">${en(p[4],3)}$</div><div class="${cls}">${en(p[5],2)}%</div></div>` } document.getElementById('plist').innerHTML=h||'<div style="padding:10px;text-align:center;color:#10B981">👑 جاهز REAL - اضغط تشغيل حقيقي - سيشتري في Binance حقيقي ✅<br>اقل صفقة 5$ - رصيدك '+d.real_balance+'$</div>';
    ...
    let btn=document.getElementById('btnRun'); if(d.is_running){ btn.innerText='⏸️ ايقاف REAL'; btn.style.background='#FF3344'; } else { btn.innerText='▶️ تشغيل حقيقي REAL'; btn.style.background='#10B981'; }
  }catch(e){}
}
