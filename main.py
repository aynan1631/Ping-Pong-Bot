...
HTML="""<!DOCTYPE html><html dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>V83 LUXURY FIXED</title>
...
.header{font-size:28px;font-weight:900;text-align:center;background:linear-gradient(90deg,#ffd700,#ffae00,#00ff88);-webkit-background-clip:text;-webkit-text-fill-color:transparent;filter:drop-shadow(0 0 20px #ffcc0044)}
.sub{font-size:11px;color:#a8add0;text-align:center;font-weight:800;margin:6px 0 14px;letter-spacing:0.5px}
...
.btn{padding:11px 18px;border-radius:12px;border:0;font-weight:900;font-size:13px;cursor:pointer;transition:0.2s}
...
.val{font-size:19px;font-weight:900;margin-top:6px}
.ltr{direction:ltr;display:inline-block;font-family:'JetBrains Mono',monospace;font-weight:800}
.g{color:#00ff88}.r{color:#ff3d57}
.table-wrap{background:linear-gradient(145deg,#1e2147,#151833);border-radius:22px;overflow:hidden;border:1.5px solid #ffffff18;margin-top:12px;box-shadow:0 10px 40px rgba(0,0,0,0.4)}
...
.badge-long{background:linear-gradient(145deg,#00e676,#00c853);color:#000;padding:6px 14px;border-radius:24px;font-size:11px;font-weight:900;min-width:62px;display:inline-block;box-shadow:0 2px 10px #00ff8844}
.badge-short{background:linear-gradient(145deg,#ff1744,#d50000);color:#fff;padding:6px 14px;border-radius:24px;font-size:11px;font-weight:900;min-width:62px;display:inline-block;box-shadow:0 2px 10px #ff3d5744}
.pill{display:inline-flex;align-items:center;gap:8px;background:linear-gradient(145deg,#1e2147,#151833);padding:12px 16px;border-radius:14px;border:1.5px solid #ffffff15;box-shadow:0 4px 15px rgba(0,0,0,0.2)}
...
<div class="header">👑 V83 TURBO LUXURY FIXED</div>
<div class="sub">حماية 3$ من القمة | يقفل الكل ويوقف 15 دقيقة ويرجع لحاله | تصميم فخم</div>

<div id="protectBox" class="protect">🛑 حماية الأرباح مفعلة</div>
...
<span style="background:linear-gradient(145deg,#00ff88,#00c853);color:#000;padding:10px 16px;border-radius:12px;font-weight:900;font-size:12px;box-shadow:0 4px 15px #00ff8844">النشط: {{strategies[s.strategy]}} | القمة: {{'%.2f'|format(s.peak_profit)}}$ | 🛡️{{s.protection_hits}}</span>
</div>
</div>
...
<div class="cardx gold"><div class="lbl">💎 الإجمالي | قمة: {{'%.2f'|format(s.peak_profit)}}$</div><div class="val" id="equity"><span class="ltr">{{'%.2f'|format(s.total_capital + s.realized + s.unrealized)}}$</span><br><span class="ltr" style="font-size:12px;color:#ffcc00" id="peak">قمة: {{'%.2f'|format(s.peak_profit)}}$</span></div></div>
...
<button class="btn btn-gold" onclick="fetch('/force_resume').then(()=>location.reload())">🚀 فك التعليق الآن</button>
</div>
...
function refresh(){
 fetch('/api').then(r=>r.json()).then(d=>{
  ...
  if(d.paused_until > Date.now()/1000){
    ...
    box.innerText=`🛑 حماية 3$ مفعلة - نزل من القمة ${d.peak_profit.toFixed(2)}$ - تم تأمين الربح - الرجوع بعد ${Math.floor(left/60)}د ${left%60}ث - [زر فك التعليق يفكه فوراً]`;
