...
def get_volatile_coins(limit=100):
    try:
        data=requests.get("https://api.binance.com/api/v3/ticker/24hr",timeout=4).json()
        cands=[]
        for d in data:
            sym=d["symbol"]
            ...
            if price>20 or price<0.0000005 or vol<4000000 or abs(ch)<1.0: continue
            if sym in cooldown and time.time()-cooldown[sym]<30: continue
            cands.append((sym,abs(ch)))
        cands.sort(key=lambda x: x[1], reverse=True)
        return [s for s,_ in cands[:limit]]
    except: return ["HEMIUSDT","PENGUUSDT","TUTUSDT","TRUMPUSDT","ASTERUSDT","WIFUSDT"]

def get_data_v2(sym):
    try:
        d1=requests.get(f"https://api.binance.com/api/v3/klines?symbol={sym}&interval=1h&limit=210",timeout=3).json()
        c1=[float(k[4]) for k in d1]
        price=c1[-1]; k=2/(201); ema=c1[0]
        for c in c1[1:]: ema=c*k+ema*(1-k)
        sma20=sum(c1[-20:])/20; sma50=sum(c1[-50:])/50
        rsi=get_rsi(c1)
        d4=requests.get(f"https://api.binance.com/api/v3/klines?symbol={sym}&interval=4h&limit=60",timeout=3).json()
...
def get_trend_tf(sym, interval):
    try:
        key = f"{sym}_{interval}"
        now = time.time()
        if key in trends_cache and now - trends_cache[key]['time'] < CACHE_TTL:
            return trends_cache[key]['trend']
        url=f"https://api.binance.com/api/v3/klines?symbol={sym}&interval={interval}&limit=60"
        data=requests.get(url, timeout=2).json()
        closes=[float(k[4]) for k in data]
        sma20=sum(closes[-20:])/20
        sma50=sum(closes[-50:])/50
        if closes[-1] > sma20 and closes[-1] > sma50: trend="UP"
        elif closes[-1] < sma20 and closes[-1] < sma50: trend="DOWN"
        else: trend=None
        trends_cache[key] = {'trend': trend, 'time': now}
        return trend
    except: return None

def get_btc_all_tf():
    try:
        if "BTC_ALL" in trends_cache and time.time() - trends_cache["BTC_ALL"]['time'] < 20:
            return trends_cache["BTC_ALL"]['data']
        data = {tf: get_trend_tf("BTCUSDT", tf) for tf in ["5m","15m","30m","1h"]}
        trends_cache["BTC_ALL"] = {'data': data, 'time': time.time()}
        return data
    except: return {"1h":"UP","30m":"UP","15m":"UP","5m":"UP"}

def is_mtf_aligned(sym, side, btc_trends):
    try:
        score=0
        trends=[]
        for tf in ["5m","15m","30m","1h"]:
            t=get_trend_tf(sym, tf)
            trends.append(f"{tf}:{t}")
            if t is None: score+=1; continue
            if side=="LONG" and t=="UP": score+=1
            if side=="SHORT" and t=="DOWN": score+=1
        # لازم 3 من 4 فريمات
        if score < 3: return False
        # BTC - لازم 1h و 30m معنا
        if side=="LONG" and btc_trends.get("1h")=="DOWN": return False
        if side=="SHORT" and btc_trends.get("1h")=="UP": return False
        return True
    except: return True

def get_all_prices_fast():
    try:
        data=requests.get("https://api.binance.com/api/v3/ticker/price",timeout=2).json()
        return {d["symbol"]: float(d["price"]) for d in data}
    except: return {}

state={"total_capital":5000.0,"per_trade":500.0,"balance":5000.0,"realized":0.0,"unrealized":0.0,"total_target":5.0,"trades":[],"TP":4.0,"SL":-6.0,"cycles":0}
...
def try_add_one():
    ...
    for sym in get_volatile_coins(100):
        ...
        d=get_data_v2(sym)
        if not d: continue
        if abs(d["price"]-d["sma20"])/d["sma20"]>0.04: continue
        side=None
        if d["price"]>d["ema"] and d["price"]>d["sma50"] and 44<d["rsi"]<63:
            side="LONG"
        elif d["price"]<d["ema"] and d["price"]<d["sma50"] and 37<d["rsi"]<56:
            side="SHORT"
        if not side: continue
        if not is_mtf_aligned(sym, side, btc_trends): continue
        score=(64-d["rsi"]) if side=="LONG" else (d["rsi"]-36)
    ...
    while True:
        try:
            ...
            if state["trades"]:
                ...
                if len(cooldown)>0 and int(time.time())%60==0:
                    cooldown.clear()
                    trends_cache.clear()
        except: pass
        time.sleep(0.8)
...
def set_tar():
    ...
    return "OK"
@app.route('/reset')
def reset(): state["realized"]=0; state["unrealized"]=0; state["trades"]=[]; state["cycles"]=0; state["balance"]=state["total_capital"]; cooldown.clear(); trends_cache.clear(); return "OK"
@app.route('/close_all')
def close_all():
    for t in state["trades"]: cooldown[t["sym"]]=time.time()
    state["realized"]=round(state["realized"]+state["unrealized"],2)
